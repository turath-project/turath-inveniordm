...
PID=$(lsof -ti:8443)
if [ ! -z "$PID" ]; then
  echo "Killing process $PID using port 8443"
  kill $PID
fi

# Start the IIIF test server in the background
echo "Starting IIIF test server on port 8443..."
python3 scripts_experimenting/iiif_test_server.py &
SERVER_PID=$!
...
echo ""
echo "IIIF test server running at: https://localhost:8443/"
